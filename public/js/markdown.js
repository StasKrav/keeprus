...
function createNoteElement(note) {
    ...
    // ============================================
    // РЕНДЕРИМ КОНТЕНТ С УМНЫМИ ССЫЛКАМИ
    // ============================================
    ...
    // 1. Сначала рендерим Markdown
    let renderedContent = renderMarkdown(contentPreview);
    
    // 2. Потом добавляем умные ссылки (если функция существует)
    ...
    const hasImage = renderedContent.includes('<img');
    
    contentWrapper.innerHTML = `
        ${hasImage ? renderedContent : `
